...
export default function Home() {
  return (
    <>
      <NavigationMenu class="fixed top-0 left-0 w-full bg-background">
        <div class="flex w-full max-w-screen-xl items-center py-2 sm:py-1 px-4">
          ...
          <div class="ml-auto">
            <Link
              href="/dashboard"
              class={cn(buttonVariants({size: "sm"}), "h-8")}
            >
              Dashboard
            </Link>
          </div>
      ...
      <main class="w-full space-y-2 pt-11">
        <section class="max-w-screen-xl mx-auto p-4 px-6 slide-in-from-bottom-10 animate-in duration-500 fade-in">
          <div class="flex flex-col gap-4 items-start transition-none h-[55vh] sm:h-[50vh] justify-center">
            <h1 class="text-balance tracking-tighter bg-gradient-to-tr  from-primary/70 via-primary to-primary/60 bg-clip-text font-bold text-transparent dark:from-muted-foreground/10 dark:muted-foreground dark:muted-foreground/20 text-4xl md:text-5xl lg:text-6xl">
              This is a concrete start
            </h1>
            <p class="text-muted-foreground text-md md:text-lg lg:text-xl max-w-2xl">
        ...
        <SSRPortal>
          <section class="w-full py-4 h-[45vh] sm:h-[50vh] fixed bottom-0 left-0">
            <Carousel opts={{loop: true}} plugins={[Autoplay({delay: 2000})]}>
              <CarouselContent class="sm:px-0 px-6">
                <For each={items}>
                  {(item) => (
                    <CarouselItem class="p-0 px-2 py-4 max-w-lg">
                      <Card class="hover:shadow-lg duration-500 h-[250px] select-none">
            ...
            <div class="absolute hidden sm:block top-0 left-0 h-full w-[20%] bg-gradient-to-r from-background to-transparent"></div>
            <div class="absolute block top-0 right-0 h-full w-[20%] bg-gradient-to-l from-background to-transparent"></div>
          </section>
        </SSRPortal>
      </main>
    </>
