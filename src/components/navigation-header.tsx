...
const NavigationMenuTrigger = <T extends ValidComponent = "button">(
  ...
  return (
    <NavigationMenuPrimitive.Trigger
      class={cn(
        "group/trigger inline-flex h-9 w-full items-center justify-center whitespace-nowrap rounded-md bg-background px-3 text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground focus:outline-none disabled:pointer-events-none disabled:opacity-50 data-[active]:bg-accent/50 data-[expanded]:bg-accent/50",
        local.class
      )}
      {...others}
    />
  ...
  return (
    ...
    >
      <NavigationMenuPrimitive.Viewport
        {...others}
        class={cn(
          "origin-[var(--kb-menu-content-transform-origin)] -ms-2 relative h-screen w-screen overflow-hidden backdrop-filter from-1% bg-gradient-to-b from-popover via-popover/20 to-popover/0 backdrop-blur-[80px] text-popover-foreground data-[expanded]:animate-in animate-out fade-out data-[expanded]:fade-in ease-in ",
          local.class
        )}
      >
        {local.children}
